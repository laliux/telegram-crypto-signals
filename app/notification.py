        ...
        if self.telegram_configured:
            ...
            for market in messages:
                ...
                for period in messages[market]:
                    ...
                    chart_file = '/tmp/charts/{}_30m.png'.format(market)
                    crop_chart_file = '/tmp/charts/crop_{}_30m.png'.format(market)

                    if os.path.exists(chart_file):
                        try:
                            try:
                                Image.open(chart_file).verify()
                                ...
                                cropped = img.crop((w2, 0, w, h))
                                #cropped = Image.new("RGB", (w2, h), "white")
                                #cropped.paste(img, (w2, 0, w, h))
                                cropped.save(crop_chart_file, "PNG")

                                self.telegram_client.send_chart(open(crop_chart_file, 'rb'), message)
                            except (IOError, SyntaxError) :
                                self.logger.info('Problemas al leer el archivo ' + chart_file)
                                self.telegram_client.notify(message)
                        except telegram.error.TimedOut:
                            self.logger.info('Error TimeOut!')
                    else:
                        self.logger.info('No existe archivo ' + chart_file)
                        self.telegram_client.notify(message)
